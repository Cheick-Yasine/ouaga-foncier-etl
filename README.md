...
1. **Scraping** (`scraper.py`) — pour chaque groupe actif de `groups.csv` : ouverture de l'URL normalisée vers `m.facebook.com`, sélection best-effort du tri « Activité récente », actualisation unique du fil avec nouvelle validation de la session, extraction des posts "mis en avant" inédits, puis scroll variable avec interception des réponses réseau GraphQL. Le navigateur et son contexte sont sauvegardés puis fermés après chaque groupe, y compris à la frontière entre deux lots.
